...
export async function POST(req){
  ...
  // Practice-mode checkpoint upsert. The client sends its own updatedAt; the
  // server refuses writes that are older than what is already stored, so a
  // delayed request can never roll progress back.
  if(body.type==="checkpoint"){
    const cp=body.checkpoint;
    if(!cp || typeof body.catId!=="string" || !Array.isArray(cp.remaining)){
      return NextResponse.json({error:"Invalid checkpoint"},{status:400});
    }
    const existing=prog.checkpoints?.[body.catId];
    if(existing?.updatedAt && cp.updatedAt && Number(cp.updatedAt)<Number(existing.updatedAt)){
      return NextResponse.json({ok:true, stale:true});
    }
    const m={ ...(prog.checkpoints||{}) };
    m[body.catId]={ ...cp, updatedAt: Date.now(), completed:false };
    prog.checkpoints=m;
    prog.markModified("checkpoints");
    await prog.save();
    return NextResponse.json({ok:true});
  }

  if(body.type==="checkpointDelete"){
    if(prog.checkpoints?.[body.catId]){
      const m={ ...(prog.checkpoints||{}) };
      delete m[body.catId];
      prog.checkpoints=m;
      prog.markModified("checkpoints");
  ...
  // Small cross-device preferences (Listen & Learn voice/speed).
  if(body.type==="prefs"){
    if(!body.prefs || typeof body.prefs!=="object") return NextResponse.json({error:"Invalid prefs"},{status:400});
    prog.prefs={ ...(prog.prefs||{}), ...body.prefs };
    prog.markModified("prefs");
    await prog.save();
    return NextResponse.json({ok:true});
  }
