...
export default function AdminPage(){
  ...
  // "/" focuses the question search (same convenience as the learner pages).
  const qSearchRef = useRef(null);
  useEffect(() => {
    const onKey = (e) => {
      if (e.key !== "/" || qModal) return;
      const tag = document.activeElement?.tagName;
      if (tag === "INPUT" || tag === "TEXTAREA" || tag === "SELECT") return;
      if (view === "subjects" && activeTab === "content") {
        e.preventDefault();
  ...
  // Applies a local patch to editCat immediately (the server round-trip for a
  // single question is small); loadCatQuiet() below re-syncs authoritative state.
  const patchEditCat = (fn) => setEditCat(cat => {
    if (!cat) return cat;
    const next = { ...cat, subcats: cat.subcats.map(sc => ({ ...sc, questions: [...sc.questions] })) };
    fn(next);
    return next;
  });
  const reindexLocal = (sc) => sc.questions.forEach((q, i) => { q.num = i + 1; });

  const submitForm = async (saveAndNew = false) => {
    if (!qModal) return;
    const payload = { ...form, correct: parseInt(form.correct) || 0 };
    const errors = {};
    ...
    setFormErrors({});
    setBusy(true);
    // Targeted endpoint: only this question is sent and mutated server-side,
    // instead of re-sending the whole subject document.
    const { ok, json: j } = await apiRequest("/api/questions/question", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        action: qModal.mode === "edit" ? "update" : "add",
        catId: editCat.id,
        subIdx: qModal.subIdx,
        num: qModal.mode === "edit" ? qModal.num : undefined,
        question: payload,
      }),
    });
    setBusy(false);
    ...
    patchEditCat(next => {
      const sc = next.subcats[qModal.subIdx];
      if (qModal.mode === "edit") {
        const idx = sc.questions.findIndex(q => q.num === qModal.num);
        if (idx !== -1) sc.questions[idx] = { ...payload, num: qModal.num };
      } else {
        sc.questions.push({ ...payload, num: sc.questions.length + 1 });
      }
    });

    if (qModal.mode === "edit") {
      flash("ok", "Question updated — saved to MongoDB.");
      closeModal();
    } else if (saveAndNew) {
      initialFormRef.current = EMPTY_FORM;
      setForm(EMPTY_FORM);
      setFormErrors({});
      flash("ok", "Question added — the form is ready for the next one.");
    } else {
      flash("ok", "Question added — saved to MongoDB.");
      closeModal();
    }
    loadCatQuiet(editCat.id);
    refreshCats();
    refreshDashStats();
  };
  ...
  // Quick answer cycling from the question table: click the answer letter to
  // rotate the correct option without opening the editor.
  const cycleAnswer = async (subIdx, num) => {
    if (!editCat) return;
    const q = editCat.subcats[subIdx]?.questions.find(qq => qq.num === num);
    if (!q) return;
    const next = (q.correct + 1) % 4;
    patchEditCat(nxt => {
      const qq = nxt.subcats[subIdx].questions.find(x => x.num === num);
      if (qq) qq.correct = next;
    });
    const { ok, json: j } = await apiRequest("/api/questions/question", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "setAnswer", catId: editCat.id, subIdx, num, correct: next }),
    });
    if (!ok) {
      flash("err", j?.error || "Could not change the answer.");
      loadCatQuiet(editCat.id); // roll back to authoritative state
      return;
    }
    flash("ok", `Correct answer set to ${String.fromCharCode(65 + next)}.`);
  };
  ...
  const runConfirm = async () => {
    ...
    if (confirmState.kind === "bulk") {
      const targets = [...selectedQs].map(k => {
      ...
      const { ok, json: j } = await apiRequest("/api/questions/question", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "bulkDelete", catId: editCat.id, targets }),
      });
      ...
      // Targeted delete: the server splices the question and renumbers the chapter.
      const { ok, json: j } = await apiRequest("/api/questions/question", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "delete", catId: editCat.id, subIdx: confirmState.subIdx, num: confirmState.num }),
      });
      if (!ok) { if (j) flash("err", j.error || "Delete failed."); setConfirmState(null); return; }
      flash("ok", "Question deleted.");
      patchEditCat(next => {
        const sc = next.subcats[confirmState.subIdx];
        if (!sc) return;
        const idx = sc.questions.findIndex(q => q.num === confirmState.num);
        if (idx !== -1) sc.questions.splice(idx, 1);
        reindexLocal(sc);
      });
              ...
              <div className="admin-panel">
                <div className="admin-panel-head">
                  <div>
                    ...
                    {editCat.description && <p style={{ marginTop: 10, fontSize: 14, color: "var(--muted)" }}>{editCat.description}</p>}
                  </div>
                ...
                <div className="admin-panel">
                  ...
                  <div className="filter-bar">
                    <input
                      ...
                      placeholder={activeChapter ? `Search within "${activeChapter.name}"…` : "Search all questions in this subject… (press /)"}
                      value={qSearch}
                  ...
                  <div className="admin-hint">Tip: click a row to edit · click the answer letter to set the correct option · press / to search</div>
                            ...
                            {pageItems.map(q => (
                              <tr key={`${q.subIdx}-${q.num}`} className={`qtable-row ${selectedQs.has(qKey(q)) ? "selected" : ""}`} onClick={() => openEditModal(q.subIdx, q)}>
                                ...
                                <td onClick={e => e.stopPropagation()}>
                                  <button
                                    className="ans-chip"
                                    title="Click to change the correct answer"
                                    aria-label={`Set correct answer for question ${q.num}`}
                                    onClick={() => cycleAnswer(q.subIdx, q.num)}
                                  >{String.fromCharCode(65 + q.correct)}</button>
                                </td>
...
function QuestionModal({ open, mode, chapterName, questionNum, form, setForm, formErrors = {}, busy, onCancel, onSubmit, onDelete }){
  const [preview, setPreview] = useState(false);
  const overlayRef = useRef(null);
  useEffect(() => {
    if (!open) { setPreview(false); return; }
    // a fresh overlay starts scrolled to the top, whatever the page scroll was
    if (overlayRef.current) overlayRef.current.scrollTop = 0;
  }, [open]);
  if (!open) return null;

  // Reorder an option (up/down); the correct-answer index follows the swap.
  const moveOption = (i, dir) => {
    const j = i + dir;
    if (j < 0 || j > 3) return;
    const options = [...form.options];
    [options[i], options[j]] = [options[j], options[i]];
    let correct = parseInt(form.correct) || 0;
    if (correct === i) correct = j;
    else if (correct === j) correct = i;
    setForm({ ...form, options, correct });
  };

  const onKeyDown = (e) => {
    if ((e.metaKey || e.ctrlKey) && e.key === "Enter") {
      e.preventDefault();
      onSubmit(mode === "add" && e.shiftKey);
    }
  };
  return (
    <div className="modal-overlay" ref={overlayRef} onMouseDown={(e) => { if (e.target === e.currentTarget) onCancel(); }}>
      <div className="modal-panel" role="dialog" aria-modal="true" aria-labelledby="qmodal-title" onKeyDown={onKeyDown}>
        <div className="modal-head">
          <div>
            <div className="mono modal-eyebrow">
              {chapterName}{mode === "edit" && questionNum != null ? ` · Question #${questionNum}` : " · New question"}
            </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <button
              type="button"
              className={`icon-action ${preview ? "active" : ""}`}
              onClick={() => setPreview(p => !p)}
              title="Show how students will see this question"
        ...
        {preview ? (
          <div className="modal-body">
            <div className="q-preview">
              <p className="question-text" style={{ fontSize: 17, whiteSpace: "pre-line" }}>{form.text || "Your question text will appear here…"}</p>
              <div className="options" style={{ marginBottom: 0 }}>
                {form.options.map((opt, i) => {
                  const isCorrect = (parseInt(form.correct) || 0) === i;
                  return (
                    <div key={i} className={`option-row ${opt.trim() ? "" : "dim"} ${isCorrect && opt.trim() ? "correct" : ""}`}>
                      <span className="option-letter">{String.fromCharCode(65 + i)}</span>
                      <span>{opt || `Option ${String.fromCharCode(65 + i)}`}</span>
                      {isCorrect && opt.trim() && <span className="stamp stamp-ok">✓ Correct</span>}
                    </div>
                  );
                })}
              </div>
            </div>
            <div className="q-preview" style={{ marginTop: 10 }}>
              <span className="label mono" style={{ display: "block", marginBottom: 6, fontSize: 10.5, letterSpacing: ".05em", textTransform: "uppercase", color: "var(--accent)" }}>Explanation students see</span>
              <span style={{ fontSize: 14, lineHeight: 1.6 }}>{form.expl || "—"}</span>
            </div>
          </div>
        ) : (
        <div className="modal-body">
          <div className="mf-field">
            ...
            <textarea className={`mf-textarea ${formErrors.text ? "mf-error" : ""}`} rows={4} placeholder="Question text" value={form.text} onChange={e => setForm({ ...form, text: e.target.value })} />
          </div>
          <div className="mf-field">
            <label className="mf-label">Options — select the correct one · use ▲▼ to reorder</label>
            {[0, 1, 2, 3].map(i => (
              <div key={i} className="mf-option-row">
                ...
                <div className="opt-tools">
                  <button type="button" className="opt-move" title="Move option up" disabled={i === 0} onClick={() => moveOption(i, -1)}>▲</button>
                  <button type="button" className="opt-move" title="Move option down" disabled={i === 3} onClick={() => moveOption(i, 1)}>▼</button>
                </div>
              </div>
            ))}
          </div>
          <div className="mf-field" style={{ marginBottom: 4 }}>
            ...
            <textarea className={`mf-textarea ${formErrors.expl ? "mf-error" : ""}`} rows={3} placeholder="Why this answer is correct" value={form.expl} onChange={e => setForm({ ...form, expl: e.target.value })} />
          </div>
          <div className="mf-hint">Saved permanently to MongoDB · Esc to cancel · Ctrl/Cmd+Enter to save{mode === "add" ? " · Ctrl/Cmd+Shift+Enter to save & add another" : ""}.</div>
        </div>
        )}
        ...
        <div className="modal-foot">
          ...
          {mode === "add" && (
            <button className="btn small secondary" disabled={busy} onClick={() => onSubmit(true)} title="Save this question and start the next one immediately">
              Save &amp; New
            </button>
          )}
          <button className="btn small" disabled={busy} onClick={() => onSubmit(false)}>{busy ? <span className="spinner"></span> : (mode === "edit" ? "Save Changes" : "Add Question")}</button>
        </div>
      </div>
    </div>
  );
