...
## Practice Checkpoints (Resume Where You Left Off)

Practice sessions auto-save as you answer (debounced ~1s, plus an immediate flush on
tab close / backgrounding / route change). Sessions with zero answers are never saved,
so a fresh session never shows a pointless "continue?" prompt.

- **Logged in:** the checkpoint is stored inside your Progress document
  (`checkpoints[catId]`) and follows the account across devices.
- **Guest:** the checkpoint lives in `localStorage` (`qh_practice_checkpoints`).
- **Stale-write guard:** every save carries an `updatedAt` timestamp; older writes can
  never overwrite newer progress (enforced client-side and in `/api/progress`).
- **Where it appears:** a resume card on the subject page, a "Continue practicing"
  dashboard on the homepage, and a "Continue your practice session?" prompt when
  re-entering the same chapter + range. Finishing or restarting clears the checkpoint.

## Listen & Learn — Cloud Text-to-Speech (optional but recommended)
...
same two neural voices everywhere:

```env
# Azure Speech (preferred) — create a Speech Service resource in the Azure portal
AZURE_TTS_KEY=your_key
AZURE_TTS_REGION=centralindia   # e.g. centralindia / eastus / westeurope
# Female voice: en-IN-NeerjaNeural · Male voice: en-IN-PrabhatNeural

# or Google Cloud Text-to-Speech
GOOGLE_TTS_API_KEY=your_api_key
# Female voice: en-IN-Neural2-C · Male voice: en-IN-Neural2-B
```

- Keys stay server-side; the browser only ever talks to `/api/tts`.
- Audio is cached on disk (`.tts-cache/`, git-ignored) keyed by provider+voice+text,
  plus a small hot in-memory map — repeated playback never re-bills the provider.
- Voice + speed preferences sync with the account (`prefs` on the Progress document);
  guests persist them in `localStorage`.
- **Without a provider** the app falls back to the device speech engine, pinned to the
  best available Indian-English voices, so the feature still works everywhere.
