...
// Build the spoken sections for one MCQ. Returns segments in play order;
// `gap` is the silence after the segment (ms) used by the fallback player.
export function getListenSegments(item, qIdx, { includeOptions = true } = {}) {
  const q = item.q;
  ...
  return segments
    .filter((s) => includeOptions || s.key !== "options")
